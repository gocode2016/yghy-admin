...
class BasicLayout extends React.PureComponent {
  ...
  getNavMenuItems(menusData, parentPath = '') {
    ...
    return menusData.map((item) => {
      ...
      if (item.subMenu && item.subMenu.some(child => child.title)) {
        return (
          <SubMenu
            title={
              item.icon ? (
          ...
          >
            {this.getNavMenuItems(item.subMenu, itemPath)}
          </SubMenu>
        );
      }
  ...
  getPageTitle() {
    ...
    let title = '阳光花艺后台管理系统';
    getRouteData('BasicLayout').forEach((item) => {
      if (item.path === pathname) {
        title = `${item.name} - 阳光花艺后台管理系统`;
      }
    });
    return title;
  }
  ...
  // 渲染
  render() {
    const { collapsed, fetchingNotices, getRouteData,menus } = this.props;
        ...
        <Layout>
          ...
          <Content style={{ margin: '24px 24px 0', height: '100%' }}>
            ...
            <GlobalFooter
              ...
              copyright={
                <div>
                  Copyright <Icon type="copyright" /> 2017 阳光花艺后台管理系统
                </div>
              }
            />
